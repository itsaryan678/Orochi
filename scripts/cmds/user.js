...
module.exports = {
	config: {
		name: "user",
		version: "1.5",
		author: "NTKhang, Modified by Rômeo",
		countDown: 5,
		role: 2,
		description: {
		...
		guide: {
			vi: " {pn} [find | -f | search | -s] <tên cần tìm>: tìm kiếm người dùng trong dữ liệu bot bằng tên"
				...
				+ "\n"
				+ "\n {pn} banlist: để xem danh sách tất cả người dùng bị cấm sử dụng bot",
			en: " {pn} [find | -f | search | -s] <name to find>: search for users in bot data by name"
				...
				+ "\n"
				+ "\n {pn} banlist: to view a list of all users banned from the bot"
		}
	},
	...
	langs: {
		vi: {
			...
			banlistEmpty: "Không có người dùng nào bị cấm trong hệ thống bot.",
			banlist: "Danh sách người dùng bị cấm:\n%1"
		},
		en: {
			...
			banlistEmpty: "No users are banned in the bot system.",
 ...
 return message.reply(getLang("banlistEmpty"));
 }

 const banlistMsg = bannedUsers.reduce((i, user, index) => i += `\n👤 **User ${index + 1}:**\n ├ ℹ Name: ${user.name}\n ├ 🆔 ID: ${user.userID}\n ├ 🚫 Reason: ${user.banned.reason}\n └ 📅 Date: ${user.banned.date}`, "");
 
 message.reply(getLang("banlist", banlistMsg));
 break;
}
			default:
				return message.SyntaxError();
